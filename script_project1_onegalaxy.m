% This script simulates the one galaxy, represented using the Toomre Model
...
for k = 1:nt
    ...
    xlim([(xmax+xmin)/2 - maxrange/2, (xmax+xmin)/2+maxrange/2]);
    ylim([(ymax+ymin)/2 - maxrange/2, (ymax+ymin)/2+maxrange/2]);
